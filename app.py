...
if st.button("Predict Optimal Course"):
    ...
    strong_str = ", ".join(strong_subjects[:3]) if strong_subjects else "core credit prerequisites"
    
    st.caption(
        f"**Reasoning:** Recommended under the **{tier_label}** classification. "
        f"Evaluated on O'Level profile (**{strong_str}**), UTME subject combination ({', '.join(utme_selected_subjects)}), "
